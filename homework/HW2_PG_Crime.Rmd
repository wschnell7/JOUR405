...
One important category of crime is accidents because it had the most reported cases in all 12 months of the year. Theft and theft from auto both saw a high number of cases in all months that had reported data. I think that resedential robbery is a very important category because of the amount of valuable that people have in ther homes. It was god to see that resedential robbery had the lowest amount of reported cases each month. 
...
```{r}
total_incedents <- pg_crime |>
  mutate(total_incidents =  rowSums(across(2:13))) |> 
  mutate(average_incedents = total_incidents/12)
```
...
Accidents, auto stolen and theft from auto are the three crime categories that have the highest average incedents per month. Homocide is the category with the lowest average incedents per month. One category I have a question about is theft. In october, there were 363 incedents of theft which is about 60 incedents higher than the second highest month so I was wondering why there was such an increase in theft incedents in October. 
...
mutate(june_change = (pg_crime(Jun) - avg_incedents)/avg_incedents *100) |>
mutate(july_change = (pg_crime(Jul) - avg_incedents)/avg_incednets *100) |>
mutate(august_change = (pg_crime(Aug) - avg_incedents)/avg_incedents *100) |>
mutate(summer_crime_averages = june_change + july_change + august_change)
```
...
It shows that the theory is not true that crime is more likely to occur in the summer. This is because there is no consistent sdata with the percent change of number of incedents that occured during the three months of the summer. 
...
Accidents are the most committed crime incedent in Prince Goerge's county by a substantial amount in 2024, according to a Prince George Country crime dataset. 
