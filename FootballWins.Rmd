...
Yes this is a useful model because the coefficients for each independent variable based on the dependent variable shows the relationship between the two which gives us a good idea of how one variable is affected by another. The model fits is a good source that provides information on the relationships between each independent variable and the dependent variable.  
...
When looking at the correlation matrix and the visual plot, it is clear that points scored has the most impact on wins. When looking at the correlation matrix, points scored has the highest correlation coefficient with each of the toher variables, Also, on the visual graph, points scored has the biggest and darkest circles when it is lined up with other variables. Yards gained seems to be the least impactful varaible based on the correlation matrix and the visual graph because it has the lowest values and the smallest and lightest circles when compared to the other variables.

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

You can use this model to cover a team by looking at the strengths of the team. If the team does well in the yards gained category, that means they have a strong offense. If a team does well in the takeaway department, that means the team has a good defense. You can take these results and compare them to each game played to see if the variables have an affect on the outcome of a game. 